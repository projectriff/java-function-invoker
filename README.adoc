...
$ docker run -ti -p 8080:8080 -v `pwd`/target/test-classes:/classes projectriff/java-function-invoker:0.0.5-sr --function.uri='file:classes?handler=io.projectriff.functions.Doubler'
```
